...
class TestInsightLog(TestCase):
    ...
    def test_get_web_requests(self):
        ...
        base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        file_name = os.path.join(base_dir, 'logs-samples/nginx1.sample')
        lines = filter_data('192.10.1.1', filepath=file_name)

        requests = [r for line in lines if (r := get_web_request(line, nginx_settings['request_model']))]
        ...
        apache2_settings = get_service_settings('apache2')
        file_name = os.path.join(base_dir, 'logs-samples/apache1.sample')
        lines = filter_data('127.0.1.1', filepath=file_name)

        requests = [r for line in lines if (r := get_web_request(line, apache2_settings['request_model']))]
        ...
        self.assertEqual(requests[0]['DATETIME'], '2016-05-04 11:31:39', "get_web_requests#6")
    ...
    def test_get_auth_requests(self):
        ...
        lines = filter_data('120.25.229.167', filepath=file_name)
        lines = filter_data(date_filter, data=lines)  # ← Liste rein, Liste raus ✓

        requests = [r for line in lines if (r := get_auth_request(line, auth_settings['request_model']))]
